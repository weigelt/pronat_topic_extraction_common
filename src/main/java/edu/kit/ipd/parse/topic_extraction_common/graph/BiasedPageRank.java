...
/**
 * PageRank implementation. Adapted by Jan Keim to fit a biased version. Could not use inheritance, as original class
 * was final...
 *
 * <p>
 ...
 *
 * <p>
 * Each iteration of the algorithm runs in linear time O(n+m) when n is the number of nodes and m the number of edges of
...
public final class BiasedPageRank<E> implements VertexScoringAlgorithm<WikiVertex, Double> {
	 ...
	 *            the maximum number of iterations to perform
	 * @param tolerance
	 *            the calculation will stop if the difference of PageRank values between iterations change less than
	 *            this value
	 */
	public BiasedPageRank(Graph<WikiVertex, E> g, double dampingFactor, int maxIterations, double tolerance,
			List<WikiVertex> senses) {
		this.g = g;
	...
	private void run(double dampingFactor, int maxIterations, double tolerance) {
		...
		if (this.g instanceof DirectedGraph<?, ?>) {
			specifics = new DirectedSpecifics((DirectedGraph<WikiVertex, E>) this.g);
		} else {
			specifics = new UndirectedSpecifics(this.g);
		}

		final int totalVertices = this.g.vertexSet().size();
		final boolean weighted = (this.g instanceof WeightedGraph<?, ?>);
		...
		for (final WikiVertex v : this.g.vertexSet()) {
			this.scores.put(v, initScore);
			if (weighted) {
				double sum = 0;
		...
		while ((maxIterations > 0) && (maxChange >= tolerance)) {
			...
			for (final WikiVertex v : this.g.vertexSet()) {
				if (specifics.outgoingEdgesOf(v).size() > 0) {
					r += (1d - dampingFactor) * this.getBias(v) * this.scores.get(v);
				} else {
					r += this.scores.get(v);
				}
			}
			...
			for (final WikiVertex v : this.g.vertexSet()) {
				...
				if (weighted) {
					for (final E e : specifics.incomingEdgesOf(v)) {
						final WikiVertex w = Graphs.getOppositeVertex(this.g, e, v);
						contribution += (dampingFactor * this.scores.get(w) * this.g.getEdgeWeight(e)) / weights.get(w);
					}
				} else {
					for (final E e : specifics.incomingEdgesOf(v)) {
						final WikiVertex w = Graphs.getOppositeVertex(this.g, e, v);
						contribution += (dampingFactor * this.scores.get(w)) / specifics.outgoingEdgesOf(w).size();
					}
				}

				final double vOldValue = this.scores.get(v);
			...
			// swap scores
			final Map<WikiVertex, Double> tmp = this.scores;
	...
	private double getBias(WikiVertex v) {
		// TODO tune this
		if (this.senses.contains(v)) {
			return 1.0 / this.senses.size();
		} else {
			return 0;
		}
	}
